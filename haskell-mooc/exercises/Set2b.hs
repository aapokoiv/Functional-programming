...
oddFactorial 2 = 1
oddFactorial 1 = 1
oddFactorial x = if even x then ((x-1) * oddFactorial ((x-1)-2)) else (x * oddFactorial (x-2))
...
myGcd a b = if a == b
    then a
    else if a < b then myGcd a (b-a) else myGcd (a-b) b
...
leftpad str x = if length str >= x
    then str
    else ((replicate y ' ') ++ str) where y = (x - length str)
...
osa 1 = show 1 ++ "... "
osa x = show x ++ "... " ++ osa (x-1)
countdown x = "Ready! " ++ str ++ "Liftoff!"
    where str = osa x
...
smallestDivisor x = smallestDivisor' 2 x
smallestDivisor' :: Integer -> Integer -> Integer
smallestDivisor' n x = if mod x n == 0 then n else smallestDivisor' (n+1) x
...
biggestPrimeAtMost x = if isPrime x == True then x else biggestPrimeAtMost (x-1)
